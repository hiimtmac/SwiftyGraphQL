...
class NodeTests: XCTestCase {
    ...
    func testOneLevel() {
        ...
        XCTAssertEqual(node.rawQuery, "myNode { hello hi }")
    ...
    func testTwoLevel() {
        ...
        XCTAssertEqual(node.rawQuery, "myNode { hello hi two { maybe no one { hi yes } } }")
        XCTAssert(node.fragmentTypes.isEmpty)
    }
    
    func testWithParameters() {
        let node = GraphQLNode.node(name: "myNode", arguments: ["since": 4, "name": "taylor"], [.attributes(["hello", "hi"])])
        XCTAssertEqual(node.rawQuery, "myNode(name: \"taylor\", since: 4) { hello hi }")
        XCTAssert(node.fragmentTypes.isEmpty)
    }
    ...
    func testOneLevelFragment() {
        ...
        XCTAssertEqual(node.rawQuery, "myNode { hello hi ...frag2 }")
    ...
    func testTwoLevelFragment() {
        ...
        let node = GraphQLNode.node(name: "myNode", [.attributes(["hello", "hi"]), two])
        XCTAssertEqual(node.rawQuery, "myNode { hello hi two { maybe no one { hi yes ...fragment1 } ...frag2 } }")
    ...
    func testNodeArray() {
        ...
        let nodes = [one, two]
        XCTAssertEqual(nodes.rawQuery, "one { ...fragment1 } two { maybe no ...frag2 }")
    ...
    func testEmptyNode() {
        ...
        XCTAssertEqual(one.rawQuery, "one")
    ...
    func testEmptyNodeWithOtherNode() {
        ...
        let two = GraphQLNode.node(name: "two", [.attributes(["no", "maybe"]), .fragment(Frag2.self)])
        let nodes = [one, two]
        XCTAssertEqual(nodes.rawQuery, "one two { maybe no ...frag2 }")
